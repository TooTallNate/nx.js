...
<Callout type="warn">
Web browser applets require **Application mode**. Your app must be running as an installed NSP, or launched from hbmenu while holding <Icon></Icon> over a game (title override).
</Callout>
...
<Files>
  <Folder name="htmldoc" defaultOpen>
...
For the full list of available options, see the [`WebAppletOptions`](/runtime/api/namespaces/Switch/interfaces/WebAppletOptions) API reference.

## RPC Pattern

A common pattern is to use JSON-based RPC between your app and the browser UI:

```typescript title="romfs/main.ts"
// nx.js side
const applet = new Switch.WebApplet('htmldoc:/index.html');
...
```javascript title="htmldoc/app.js"
// Browser side
let msgId = 0;
const pending = {};
