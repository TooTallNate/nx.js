/**
 * Stub implementations for functions referenced by wasm.c
 * that normally live in other nx.js source files or libnx.
 *
 * These stubs allow the WASM test harness to compile and link on a host
...
JSValue nx_throw_wasm_error(JSContext *ctx, const char *name, const char *r);
