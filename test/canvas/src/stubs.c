/**
 * Stub implementations for functions referenced by canvas.c, font.c, image.c,
 * error.c that normally live in other nx.js source files or libnx.
 *
 * These stubs allow the canvas test harness to compile and link on a host
 * system without any Nintendo Switch SDK.
 */

// Use the real types.h from source/ — compat/ provides stubs for
// <switch.h>, <wasm3.h>, <mbedtls/*.h> so the real types.h compiles on host.
#include "types.h"
#include <errno.h>
#include <stdio.h>
#include <string.h>
...
void nx_init_error(JSContext *ctx, JSValueConst init_obj) {
	(void)ctx;
	(void)init_obj;
	// no-op on host — error handler registration not needed for tests
}
...
void nx_process_async(JSContext *ctx, nx_context_t *nx_ctx) {
	(void)ctx;
	(void)nx_ctx;
	// no-op — async processing not used in test host
}
