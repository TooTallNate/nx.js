...
static JSValue nx_new_font_face(JSContext *ctx, JSValueConst this_val, int argc,
								JSValueConst *argv) {
	...
	FT_Error ft_err = FT_New_Memory_Face(nx_ctx->ft_library,
	...
	// For Harfbuzz, load using OpenType (HarfBuzz FT does not support bitmap
	// font)
...
static JSValue nx_get_system_font(JSContext *ctx, JSValueConst this_val,
