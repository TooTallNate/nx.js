...
static JSValue nx_account_exit(JSContext *ctx, JSValueConst this_val, int argc,
							   JSValueConst *argv) {
	accountExit();
	return JS_UNDEFINED;
}

static JSValue nx_account_initialize(JSContext *ctx, JSValueConst this_val,
...
static JSValue nx_account_current_profile(JSContext *ctx, JSValueConst this_val,
										  int argc, JSValueConst *argv) {
	AccountUid uid;
...
static JSValue nx_account_select_profile(JSContext *ctx, JSValueConst this_val,
										 int argc, JSValueConst *argv) {
	AccountUid uid;
...
static JSValue nx_account_profiles(JSContext *ctx, JSValueConst this_val,
								   int argc, JSValueConst *argv) {
	Result rc;
	s32 user_count;
...
static JSValue nx_account_nickname(JSContext *ctx, JSValueConst this_val,
								   int argc, JSValueConst *argv) {
	Result rc;
...
static JSValue nx_account_image(JSContext *ctx, JSValueConst this_val, int argc,
								JSValueConst *argv) {
	Result rc;
	nx_profile_t *profile = nx_get_profile(ctx, this_val);
	if (!profile) {
	...
	if (R_FAILED(rc)) {
		JS_ThrowInternalError(ctx, "accountProfileGetImageSize() returned 0x%x",
							  rc);
		return JS_EXCEPTION;
	}
	...
	rc = accountProfileLoadImage(&profile->profile, buf, image_size,
								 &image_size);
	if (R_FAILED(rc)) {
		JS_ThrowInternalError(ctx, "accountProfileLoadImage() returned 0x%x",
							  rc);
		return JS_EXCEPTION;
	}
...
static JSValue nx_account_profile_init(JSContext *ctx, JSValueConst this_val,
									   int argc, JSValueConst *argv) {
	JSAtom atom;
