...
static JSValue nx_ns_app_new(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
	...
	if (JS_IsNull(argv[0]))
	{
		...
		// This is the case for NSP installations, or when running via an emulator (i.e. Ryujinx).
		Result rc = svcGetInfo(&application_id, InfoType_ProgramId, CUR_PROCESS_HANDLE, 0);
		if (R_FAILED(rc))
		{
	...
	else
	{
		...
		memcpy(&data->data.icon, asset_header + icon_section_offset, icon_section_size);
		memcpy(&data->data.nacp, asset_header + nacp_section_offset, nacp_section_size);
	...
	{
		size_t outSize;
		Result rc = nsGetApplicationControlData(NsApplicationControlSource_Storage, application_id, &data->data, sizeof(data->data), &outSize);
		if (R_FAILED(rc))
		{
			return JS_ThrowInternalError(ctx, "nsGetApplicationControlData() returned 0x%x", rc);
		}
...
static JSValue nx_ns_app_id(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
...
static JSValue nx_ns_app_author(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
	nx_app_t *app = nx_get_app(ctx, this_val);
	if (!app)
	{
...
static JSValue nx_ns_app_next(JSContext *ctx, JSValueConst this_val, int argc, JSValueConst *argv)
{
