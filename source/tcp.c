...
void nx_on_read(nx_poll_t *p, nx_read_t *req) {
	...
	if (req->err) {
		...
		arg = JS_NewInt32(ctx, req->bytes_read);
	}
...
JSValue nx_js_tcp_server_close(JSContext *ctx, JSValueConst this_val, int argc,
							   JSValueConst *argv) {
	nx_context_t *nx_ctx = JS_GetContextOpaque(ctx);
...
static JSValue nx_js_tcp_init_server(JSContext *ctx, JSValueConst this_val,
