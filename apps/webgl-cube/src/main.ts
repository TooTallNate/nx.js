...
if (!gl) {
	// WebGL2 requires application mode (the full memory grant). In applet
	// mode (launched from the Album) there is not enough memory for the GL
	// driver, so `getContext('webgl2')` returns null.
	console.log('Failed to create a WebGL2 context.');
	console.log('');
	console.log('WebGL2 requires application mode. Launch hbmenu with the');
	console.log('title override (hold R while opening a game) and try again.');
	console.log('');
...
function main(gl: WebGL2RenderingContext) {
	// NOTE: prefer `console.printErr()` (debug log only, no rendering) over
	// `console.log()` for diagnostics in WebGL apps — the console's canvas
	// terminal is not visible while the GL context owns the screen anyway.
	console.printErr(`${gl.getParameter(gl.VERSION)}\n`);
	console.printErr(`Renderer: ${gl.getParameter(gl.RENDERER)}\n`);
