...
- **The console is themeable via `console.options` (or `new Console(opts)`).** `ConsoleOptions extends TerminalOptions` (`theme`, `fontSize`, `lineHeight`, `scrollback`, `cursorStyle` `'block'|'underline'|'bar'`, `cursorOpacity`). For the *global* `console`, assign `console.options` BEFORE the first log — the `Terminal` is created lazily in `#getTerminal()`, and the setter drops any existing terminal so the next output rebuilds it (scrollback is reset). The renderer honors the **full** xterm ANSI palette: `terminal.ts` resolves a 16-entry `#palette` from the theme's `black`..`brightWhite` fields over the `ANSI_COLORS` defaults (don't re-hardcode `ANSI_COLORS` in `#cellColor` — index `#palette`). See `apps/console-theme` for a Solarized Dark example.
